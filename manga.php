...
?>
	...
	<body>
		...
		<nav id="nav" class="navbar">
			...
			<div class="nav_container">
				<div class="nav_elements">
					...
					<li class="active"> <a class="elements" href="manga.php"> Manga </a> </li>
		...
		<div id="main_wrap">
			...
			<?php
				function getVotes($result, $utenti, $votaremanga, $manga, $conn) {
					$lista_finale = "";
					$sql2 = "WITH cte AS ( SELECT v.* FROM $manga f JOIN $votaremanga v ON v.Cod = f.Cod WHERE f.Titolo = '".$result["Titolo"]."') SELECT * FROM $utenti u LEFT JOIN cte ON cte.Id = u.Id ORDER BY voto DESC";
					/*$sql2 = "SELECT * FROM $utenti LEFT OUTER JOIN $votaremanga ON $utenti.ID = $votaremanga.ID
												  LEFT OUTER JOIN $manga ON votare.Cod = $manga.Cod
				...
				$sql = "SELECT ROUND(AVG(Voto),2) AS Media ,Titolo,img_name, $manga.Cod, sinossi FROM $utenti INNER JOIN $votaremanga ON $utenti.ID = $votaremanga.ID
						RIGHT OUTER JOIN $manga ON $votaremanga.Cod = $manga.Cod
						GROUP BY Titolo ORDER BY Media DESC";
				$query = mysqli_query($conn, $sql);
				$result = mysqli_fetch_array($query);
				if($result != null) {
					$i = 0;
					echo "<div id=\"containermanga\">";
					do {
						if($i % 4 == 0 && $i!=0) {
							echo "</div>";
							echo "<div id=\"containermanga\">";
						...
						if($voto_medio == NULL)
							$voto_medio = 0;
						echo "<div class=\"cardmanga_padding_rightleft\">
							<div id=\"card_title\">
						...
						<div class=\"cardmanga\">
						  <div class=\"overlay\">
							...
							<img onclick= \"openmodal(".$result["Cod"].")\" id=\"".$result["Titolo"]."\"class=\"imgcard\" src=\"imgmanga/".$result["img_name"]."\">
							...
							<div id=\"".$result["Cod"]."\" class=\"modal\">
							  <div class=\"modal-content\">
								...
								<div id=\"votes_list\">
							    <h2>".$result["Titolo"]."</h2>".
								getVotes($result, $utenti, $votaremanga, $manga, $conn)
								."</div>
								  <div class=\"sinossi\"".$result["Cod"]."\">
									".$result["sinossi"]."
								  </div>
							  </div>
							</div>
